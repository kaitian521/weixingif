...
MagickExport MagickBooleanType FileToImage(Image *image,const char *filename)
{
  ...
  unsigned char
    *blob;
	  ...
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "11111111111111111111");
  ...
  if (file == -1)
    {
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "UnableToOpenBlob");
      ThrowFileException(&image->exception,BlobError,"UnableToOpenBlob",
        filename);
      return(MagickFalse);
    }
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "2222222222222222222");
  quantum=(size_t) MagickMaxBufferExtent;
  if ((fstat(file,&file_info) == 0) && (file_info.st_size != 0))
  ...
  if (blob == (unsigned char *) NULL)
    {
	   __android_log_print(ANDROID_LOG_ERROR, "JNITag", "MemoryAllocationFailed");
      ThrowFileException(&image->exception,ResourceLimitError,
        "MemoryAllocationFailed",filename);
      return(MagickFalse);
    }
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "333333333333333333333");
  for ( ; ; )
  {
    ...
    count=WriteBlobStream(image,length,blob);
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "44444444444444444444444");
    if (count != (ssize_t) length)
      {
		__android_log_print(ANDROID_LOG_ERROR, "JNITag", "MemoryAllocationFailed");
  ...
  file=close(file);
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "5555555555555555555");
  if (file == -1) {
	__android_log_print(ANDROID_LOG_ERROR, "JNITag", "UnableToWriteBlob");
    ThrowFileException(&image->exception,BlobError,"UnableToWriteBlob",
      filename);
  }
  blob=(unsigned char *) RelinquishMagickMemory(blob);
  ...
  if (*filename == '|')
    {
      ...
      *mode=(*type);
      ...
      if (image->blob->file == (FILE *) NULL)
        {
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "3222222222222222222222222");
  ...
  if ((status == MagickTrue) && S_ISFIFO(image->blob->properties.st_mode))
    {
      ...
      if (image->blob->file == (FILE *) NULL)
        {
	  __android_log_print(ANDROID_LOG_ERROR, "JNITag", "33333333333333333333333");
          ThrowFileException(exception,BlobError,"UnableToOpenBlob",filename);
          return(MagickFalse);
        }
